/*
  ...
  1. New Functions
    - `update_incident_verification_count()` - Updates verification count and status
    - `calculate_incident_verification_score()` - Calculates weighted verification score

  2. New Triggers
    - `trigger_update_incident_verification` - Fires after incident verification insert/update/delete

  3. Security
    - Functions are security definer to ensure proper execution
...
BEGIN
  ...
  BEGIN
    ...
    SELECT 
      COUNT(*) FILTER (WHERE verification_type = 'confirm') as confirms,
      COUNT(*) FILTER (WHERE verification_type = 'dispute') as disputes,
      COUNT(*) as total
    ...
    -- Require at least 3 confirmations and positive score
...
BEGIN
  ...
  SELECT 
    ...
    i.is_verified,
    CASE 
      WHEN COALESCE(v.total_verifications, 0) = 0 THEN 0
...
BEGIN
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION 'User has already verified this incident. Use UPDATE to change verification.';
  END IF;
...
BEGIN
  FOR incident_record IN 
    SELECT DISTINCT incident_id FROM incident_verifications
  LOOP
    PERFORM update_incident_verification_count() 
    WHERE NEW.incident_id = incident_record.incident_id;
  END LOOP;
  
  RAISE NOTICE 'Updated verification counts for existing incidents';
END $$;
